...
class BitMEX(object):
    ...
    def market_depth(self, symbol):
        """Get market depth / orderbook.不再支持"""
        return self.ws.market_depth(symbol)

    def recent_trades(self):
        """Get recent trades.返回实时交易
    ...
    def get_last_trade(self, symbol, count, columns='price,side,size', filter=None):
        """返回最近交易, 倒叙排列"""
        if(filter != None):
            query = {
                'symbol': symbol,
        ...
        return self._curl_bitmex(path='trade', query=query, verb='GET')

    #
    # Authentication required methods
    #
    def authentication_required(fn):
        """Annotation for methods that require auth.要求进行身份验证"""
    ...
    @authentication_required
    def buy_stop(self, quantity, price):
        """Place a buy stop order.

        Returns order object. ID: orderID
        """
        return self.stop_order(quantity, price, 'Buy')

    @authentication_required
    def sell_stop(self, quantity, price):
        """Place a sell stop order.

        Returns order object. ID: orderID
        """
        return self.stop_order(-quantity, price, 'Sell')

    @authentication_required
    def stop_order(self, quantity, price, side):
        """Stop Market order.创建设置止损单"""
        if price < 0:
            raise Exception("Price must be positive.")
    ...
    def cancel(self, orderID):
        """Cancel an existing order.取消一个存在的订单"""
        path = "order"
        postdict = {
            'orderID': orderID,
        }
    ...
    def cancel_all_orders(self):
        """Cancels all of your orders.取消所有存在的订单"""
        path = "order/all"
        return self._curl_bitmex(path=path, verb="DELETE")
    ...
    def _curl_bitmex(self, path, query=None, postdict=None, timeout=None, verb=None, rethrow_errors=False,
        ...
        if max_retries is None:
            max_retries = 200 if verb in ['POST', 'PUT'] else 600
                ...
                if 'duplicate clordid' in message:
                    ...
                    for i, order in enumerate(orderResults):
                        if (
                                ...
                                order['stopPx'] != postdict['stopPx'] or
