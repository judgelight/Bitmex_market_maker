...
class CustomOrderManager(OrderManager):
    ...
    def place_orders(self):
        ...
        if(self.running_qty == 0 and wave_coefficient < 8 and (self.last_running_qty != 0 or self.reset == True)):
            sleep(5)
            for i in reversed(range(1, 4 * (settings.ORDER_PAIRS - 1) + 1 + 1)):
            ...
            cycles_buy = 1 if self.running_qty < settings.ORDER_START_SIZE else (self.running_qty - settings.ORDER_START_SIZE) // (settings.ORDER_START_SIZE // 4) + 2
            if (self.running_qty == self.last_running_qty):     #持仓不变
                ...
                order_status = 4
                for i in reversed(range(cycles_buy, 4 * (settings.ORDER_PAIRS - 1) + 1 + 1)):
                    if not self.long_position_limit_exceeded():
                        buy_orders.append(self.prepare_order(-i, order_status))
            ...
            cycles_sell = 1 if abs(self.running_qty) < settings.ORDER_START_SIZE else (abs(self.running_qty) - settings.ORDER_START_SIZE) // (settings.ORDER_START_SIZE // 4) + 2
            if (self.running_qty == self.last_running_qty):     #持仓不变
                return
                ...
                order_status = 2
                for i in reversed(range(cycles_sell, 4 * (settings.ORDER_PAIRS - 1) + 1 + 1)):
                    if not self.short_position_limit_exceeded():
                        sell_orders.append(self.prepare_order(i, order_status))
    ...
    def prepare_order(self, index, order_status):
        ...
        if(index == 1 or index == -1):
            if (((self.running_qty > 0 and order_status == 4) or (self.running_qty < 0 and order_status == 2))) and (abs(self.running_qty) % settings.ORDER_START_SIZE) != 0:  #多仓部分减少或空仓部分减少
                ...
                quantity = settings.ORDER_START_SIZE + (abs(self.running_qty) % settings.ORDER_START_SIZE)   #仓位化整
