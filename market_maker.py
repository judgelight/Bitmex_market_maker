...
class ExchangeInterface:
    def __init__(self, dry_run=False, base_url=settings.BASE_URL, apiKey=settings.API_KEY, apiSecret=settings.API_SECRET, orderIDPrefix=settings.ORDERID_PREFIX, postOnly=settings.POST_ONLY, timeout=settings.TIMEOUT):
        ...
        self.bitmex = bitmex.BitMEX(base_url=base_url, symbol=self.symbol,
                                    ...
                                    orderIDPrefix=orderIDPrefix, postOnly=postOnly,
                                    timeout=timeout)
    ...
    def get_portfolio(self):
        ...
        for symbol in contracts:
            ...
            portfolio[symbol] = {
                "currentQty": float(position['currentQty']),    #我的当前仓位数量
                "futureType": future_type,
                "multiplier": multiplier,
                "markPrice": float(instrument['markPrice']),    #BitMEX 指数价格
                "spot": float(instrument['indicativeSettlePrice'])#标记价格
            }
    ...
    def calc_delta(self):
        ...
        delta = {
            "spot": spot_delta,#开仓价值
            "mark_price": mark_delta,#目前仓位价值
            "basis": basis_delta#未实现盈亏
        }
        return delta
    ...
    def get_margin(self):
        """Get your current balance.你账户的余额和保证金要求的更新"""
        if self.dry_run:
    ...
    def get_position(self, symbol=None):
        """Get your open position.你仓位的更新"""
        if symbol is None:
            symbol = self.symbol
        return self.bitmex.position(symbol)

    def get_ticker(self, symbol=None):
        """返回从instrument那里取得的买一价和卖一价
        ticker = {
    ...
    def amend_bulk_orders(self, orders):
        """Amend multiple orders.修改多个订单"""
        if self.dry_run:
            return orders
        return self.bitmex.amend_bulk_orders(orders)

    def create_bulk_orders(self, orders):
        """Create multiple orders.创建多个订单"""
        if self.dry_run:
            return orders
...
class OrderManager:
    ...
    def print_status(self):
        ...
        logger.info("Current XBT Balance: %.6f" % XBt_to_XBT(self.start_XBt))#目前可用资金
        logger.info("Current Contract Position: %d" % self.running_qty)#持仓量
        if settings.CHECK_POSITION_LIMITS:
            logger.info("Position limits: %d/%d" % (settings.MIN_POSITION, settings.MAX_POSITION))#目前允许的最大可持仓量
        if position['currentQty'] != 0:
            logger.info("Avg Cost Price: %.*f" % (tickLog, float(position['avgCostPrice'])))#开仓价格
            logger.info("Avg Entry Price: %.*f" % (tickLog, float(position['avgEntryPrice'])))#平均价格
        logger.info("Contracts Traded This Run: %d" % (self.running_qty - self.starting_qty))#持仓量
        logger.info("Total Contract Delta: %.4f XBT" % self.exchange.calc_delta()['spot'])#开仓价值

    def get_ticker(self):
        """获得买一价和卖一价，并根据此价格设置order的开始买价和卖价"""
        ...
        if settings.MAINTAIN_SPREADS is False:
            if 0 < self.exchange.get_highest_buy()['price'] <= 100000:
                self.start_position_buy = elf.exchange.get_highest_buy()['price']
            if 0 < self.exchange.get_lowest_sell()['price'] <= 100000:
                self.start_position_sell = self.exchange.get_lowest_sell()['price']
    ...
    def get_price_offset(self, index):
        """Given an index (1, -1, 2, -2, etc.) return the price for that side of the book.
           Negative is a buy, positive is a sell.根据index依次设置每一个价格，这里为等差"""
